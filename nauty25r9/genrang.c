...
main(int argc, char *argv[])
{
    ...
    if ((P1switch!=0) + (eswitch!=0) + (rswitch!=0)
                      + (Rswitch!=0) + (tswitch!=0) > 1)
        gt_abort(">E genrang: -Pert are incompatible\n");
    ...
    usesparse = tswitch || (rswitch && !aswitch && codetype==SPARSE6);
#if !MAXN
	...
	else if (tswitch) randomtree(&sg,n);
