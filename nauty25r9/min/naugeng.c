...
static void
writeg6(FILE *f, graph *g, int n)
/* write graph to file in graph6 format */
{
  writeline(f, ntog6(g, n));
}
...
static void
geng_userautomprocb(int count, int *p, int *orbits,
                    int numorbits, int stabvertex, int n)
/* form orbits on powerset of VG
   called by nauty;  operates on data[n] */
{
...
/* decide if n in theta(g+x)  --  version for n+1 < maxn */
{
  ...
  options.getcanon = TRUE;
  options.defaultptn = FALSE;
  options.userautomproc = &geng_userautomprocb;
...
#endif
{
  ...
  if (header) {
    if (!yformat && !nautyformat && !nooutput) {
